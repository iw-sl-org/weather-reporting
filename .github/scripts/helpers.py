...
def sl_checkout_to_project(sl_org, sl_pspace, sl_project, sl_auth, gh_ref):
    # This function checkouts from GitHub ref to SnapLogic project.
...
def sl_checkout_to_project_tag(sl_org, sl_pspace, sl_project, sl_auth, gh_ref, tag_name):
    # This function checkouts from GitHub tag to SnapLogic project.
...
def sl_pipeline_linter(sl_org, sl_pspace, sl_project, sl_pipeline, sl_auth):
    # This function checkouts from GitHub ref to SnapLogic project.
    print(f"-- Pipeline - {sl_pipeline} - quality check --")
    # Structure request
    url = f"https://elastic.snaplogic.com/api/1/rest/public/pipeline/linter/{sl_org}/{sl_pspace}/{sl_project}/{sl_pipeline}"
    payload = json.dumps({})
    headers = {}
    response = requests.request("POST", url, auth=sl_auth, headers=headers, data=payload)
    response = json.loads(response.text)
    # Validate response
    if response["http_status_code"] != 200:
        exit("# Invalid request - SnapLogic pipeline linter endpoint! #")
    return response["response_map"]["complete_report"]
...
def gh_list_artifacts(gh_ref, gh_token):
    ...
    # Structure request
    url = f"https://api.github.com/repos/{gh_ref}/actions/artifacts?name=tasks-config-artifact"
    payload={}
    headers = {
        "Accept": "application/vnd.github+json",
        "X-GitHub-Api-Version": "2022-11-28",
...
def pete_code_review(pete_key, pete_token, sl_org, sl_pspace, sl_project):

    # Structure request
    url = "https://api-pete.iwdg.mk/api/generate-code-review"
    headers = {
    ...
    if response.status_code != 200:
        exit("# Invalid request - Pete Code Review endpoint! #")
